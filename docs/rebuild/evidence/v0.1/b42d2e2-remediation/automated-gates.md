...
| React typecheck | `cd rebuild/apps/desktop && pnpm typecheck` | Passed |
...
| 门禁 | 命令/证据 |
|---|---|
| 固定命名空间与 Agent 布局 | `python rebuild/scripts/build_dev_package.py`（REQUIRED_LAYOUT 校验） |
...
| dev package manifest | `rebuild/dist/dev-package-manifest.json` → 证据包 `package-validation.json` |
