...
现有 `src/QuantifiedSelf.Windows.Tauri` 可以作为 UI 和托盘行为参考；迁入 `rebuild/apps/desktop` 后，不复制 `src/bridge` 和 `BridgeSupervisor`。
...
| V01-6 Desktop | Tauri Query/IPC client、CAS Settings、detached Agent、新 Desktop 不携带 bridge 代码与 BridgeSupervisor（旧 `src/QuantifiedSelf.Windows.Tauri` 树原样保留） | handshake/DTO/版本错误；React→Tauri→Rust Agent/SQLite 端到端通过 |
...
不得先删除旧 C#/WPF/Bridge 再验证替代链路。v0.1 dev package 通过后，只停止在 rebuild dev 路径使用 Bridge；旧系统仍保留。
...
两套系统不共享数据库，因此回滚不转换数据，也不会让旧 Agent 打开新库。v0.1 dev 数据可以保留作诊断或在用户确认后删除；不得自动删除旧系统数据。
...
旧系统退役仍必须等到生产版本达到所需功能、数据和稳定性门禁，v0.1 完成不等于允许删除旧代码。
