...
状态：实施状态记录（不定义设计）
基线日期：2026-07-18
最近更新：2026-07-19（V01-1、V01-2、V01-3、V01-4 完成）
...
目标架构尚未形成可运行的 Rust Agent 链路，但 v0.1 已完成四个阶段：V01-1 建立 `rebuild/` workspace 与 `wuji-core`（commit `c2ca961`）；V01-2 落地 `wuji-storage`，schema 已移入 `rebuild/crates/wuji-storage/schema/schema.sql` 作为唯一 DDL，空库 bootstrap、Writer 行操作、触及桶重算与只读 Reader 已实现；V01-3 落地 Win32 采集与隐私过滤（`wuji-windows` 适配器、`wuji-core` 流水线处理器、采集循环与 bounded queue）；V01-4 落地 Activity/Work 精确状态机（`apps/agent/activity.rs`）：规则 1–5 归属/切段/时钟处理、sampling_transition、Work Block 创建/pending idle/回溯结束、queue drop epoch、隐私/生命周期边界、gap 叠加规则与启动恢复，19 项黄金样本守恒测试（switch/idle/break/crash/DST/守恒交叉验证）全部通过。`cargo test --workspace`（76 项）、`clippy -D warnings`、`fmt --check` 全部通过。空库 bootstrap 与最小读模型已实现；Agent 运行时（Writer lane/IPC/心跳）、bridge-free Tauri 与四条 UI 路径尚未实现。
...
| Rust Agent binary | Partial | `rebuild/apps/agent`：采集循环、ObservationProcessor、ActivityEngine（09 §6.5/6.6/6.7 全规则）与 19 项黄金样本守恒测试已接入 | 独立 Rust Agent 长期进程 | 双 lane Writer、CommandServer、heartbeat、单实例未接入 | V01-5 |
...
| Named Pipe v2 | Design only | [06](./06-本地接口与错误合同.md) 为 Draft | DACL + Desktop binary/signature manifest + 内存 session capability、版本握手、幂等 receipt | 当前 Tauri 通过 Bridge 间接使用旧 C# 合同；无目标认证实现 | M6 / IPC-001–005 |
...
| 验证项 | 仓库能力 | 本基线结果 | 说明 |
|---|---|---|---|
| Rebuild `cargo test --workspace` / `cargo clippy -D warnings` / `cargo fmt --check` | 命令存在（`rebuild/`） | Passed（2026-07-19，76 项测试全过、零警告） | 覆盖 wuji-core 合同、wuji-storage 存储、wuji-windows 真实采集、Agent 采集流水线与 Activity/Work 黄金样本；随 V01 阶段扩展 |
...
5. 接受第 5–6、8 节，实现双 lane Writer、CommandServer、heartbeat、单实例与恢复（V01-5）；
