...
严重度：High  
证据：`rebuild/crates/wuji-storage/src/reader.rs`
...
严重度：High  
证据：`rebuild/apps/agent/src/activity.rs`、`writer_task.rs`、`command_server.rs`
...
严重度：High  
证据：`rebuild/apps/agent/src/main.rs`、`settings_store.rs`、`activity.rs`、`rebuild/apps/desktop/src-tauri/src/settings_service.rs`
...
严重度：High  
证据：`rebuild/apps/agent/src/command_server.rs`、`rebuild/apps/desktop/src-tauri/src/ipc.rs`
...
严重度：High  
证据：`rebuild/scripts/soak.py`、本地 `rebuild/dist/soak-report.json`
...
8. manifest 与 soak report 位于被 `.gitignore` 排除的 `rebuild/dist/`，仓库内没有可复核证据；
...
```text
docs/rebuild/evidence/v0.1/<commit>/
  manifest.json
  automated-gates.md
  soak-summary.json
