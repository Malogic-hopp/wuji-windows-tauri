...
下一步进入 **阶段 5 Settings 垂直切片**：先扩展 settings schema 和安全 DTO，再实现 Bridge 读取/保存、Rust 语义 command 与 React 表单四态/校验/失效。继续不修改现有 WPF 默认 Shell、IPC、SQLite、Agent 状态机或 WPF 发布流程。
