...
状态：阶段 0～3 已完成；阶段 4 实施中（4A 合同与 Bridge 已完成）；阶段 5～8 待实施
...
实施状态（2026-07-16）：4A 合同与 Bridge 已完成。`activity.getOverview`、安全 Dashboard DTO、C#/Rust/TypeScript 生成物、`IWujiClient.Activity.Overview` 并行聚合、安全映射和隐私门禁均已落地；Tauri command 与 React Dashboard 尚未实施。详见 `docs/devlog/Tauri2-React19阶段4A完成说明-2026-07-16.md`。
...
阶段 0～3 与阶段 4A 的实现和自动门禁已于 2026-07-16 完成：
...
9. WebView 只拥有七个语义 command，CSP 禁止远程脚本、iframe、对象和 eval，capability 不授予 shell/fs/http/sql/process；
...
下一批继续 **阶段 4：Dashboard 垂直切片**，先增加 Rust `activity_get_overview` 语义 command，再接入 React Dashboard。继续不修改现有 WPF 默认 Shell、IPC、SQLite、Agent 状态机或 WPF 发布流程。
