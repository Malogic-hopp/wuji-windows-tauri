...
下一步进入 **阶段 6 托盘、生命周期、单实例与 channel 收口**：先实现宿主退出/隐藏/托盘与 Bridge graceful shutdown，再接入单实例和 channel 门禁，最后执行 Agent 独立存活、孤儿进程、冷启动和长时间 soak 验收。继续不修改现有 WPF 默认 Shell、IPC、SQLite、Agent 状态机或 WPF 发布流程。
