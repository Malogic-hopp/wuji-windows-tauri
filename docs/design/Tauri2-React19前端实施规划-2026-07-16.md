...
状态：阶段 0～5 已完成；阶段 6A～6B 自动化实施完成、真实 dev 合并 smoke 待执行；阶段 6C～8 待实施
...
当前状态：阶段 6A 宿主生命周期与托盘骨架、阶段 6B 托盘 Agent 控制、单实例和 dev channel 收口已于 2026-07-17 完成自动化实施。关闭、隐藏到托盘、真正退出、Settings dirty 原生确认、Bridge graceful/forced shutdown、Agent 四态托盘菜单、第二实例激活已有窗口、固定 dev 身份反向校验及 dev/prod 隔离门禁均已落地；阶段 6A + 6B 真实 dev 合并 smoke 待人工执行。详见 `docs/devlog/Tauri2-React19阶段6A完成说明-2026-07-17.md` 和 `docs/devlog/Tauri2-React19阶段6B完成说明-2026-07-17.md`。
...
下一步先执行 **阶段 6A + 6B 真实 dev 合并 smoke**，确认托盘四态与 Agent 操作、隐藏/恢复、dirty 关闭确认、第二实例激活已有窗口、UI 退出后 Agent 独立存活、Bridge 无残留以及 dev/prod pipe、mutex、数据和启动项隔离；通过后进入阶段 6C，补齐实机冷启动矩阵、孤儿进程检查、WPF/Tauri 差异记录和长时间 soak。继续不修改现有 WPF 默认 Shell、IPC、SQLite、Agent 状态机或 WPF 发布流程。
