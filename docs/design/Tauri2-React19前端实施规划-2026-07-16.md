...
当前 WPF ViewModel 会先调用 `StopAgentGracefullyAsync()`，失败后再调用 `KillAgentAsFallbackAsync()`。这段停止编排不能复制到 React、Rust 或 Bridge；阶段 2 应先把它收口为 Application/Client 的统一停止用例，再由 WPF 和 Bridge 共同调用，并用回归测试冻结超时、fallback 和最终状态语义。
...
阶段 0 + 阶段 1 已于 2026-07-16 完成：

1. 阶段 7 已独立收口；
2. ADR、协议方法、错误模型和敏感字段已经冻结；
3. Bridge、Bridge.Tests、ContractGen 和 schema 目录已经建立；
4. hello、initialize、shutdown 已打通；
5. C#/Rust/TypeScript 合同生成、stdout purity、Dispose 不停 Agent 和架构门禁均已有自动测试；
6. 完整 .NET 构建和测试已通过，详见 `docs/devlog/Tauri2-React19阶段0-1完成说明-2026-07-16.md`。

下一批建议只执行 **阶段 2：Bridge Agent 生命周期切片**。开始前先完成 WPF dev 对照 smoke，再实现安全 Agent DTO 和 status/start/pause/resume/stop；仍不创建可见 React 页面，不修改 IPC、SQLite、Agent 状态机、`App.xaml.cs` 或现有 WPF 发布流程。
