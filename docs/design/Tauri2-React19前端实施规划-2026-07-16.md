...
状态：阶段 0～5 已完成；阶段 6A 自动化实施完成、人工 smoke 待执行；阶段 6B～8 待实施
...
当前状态：阶段 6A 宿主生命周期与托盘骨架已于 2026-07-17 完成自动化实施。关闭、隐藏到托盘、真正退出、Settings dirty 原生确认、Bridge graceful/forced shutdown、worker 完成等待及 Rust/React/架构门禁均已落地；真实 dev 托盘与 Agent 独立存活 smoke 待人工执行。详见 `docs/devlog/Tauri2-React19阶段6A完成说明-2026-07-17.md`。
...
下一步先执行 **阶段 6A 真实 dev 托盘 smoke**，确认隐藏/恢复、dirty 关闭确认、UI 退出后 Agent 独立存活和 Bridge 无残留；通过后进入阶段 6B 单实例/channel 收口，最后在 6C 执行冷启动和长时间 soak。继续不修改现有 WPF 默认 Shell、IPC、SQLite、Agent 状态机或 WPF 发布流程。
