...
建议下一次实施只执行 **阶段 0 + 阶段 1**：

1. 先收口并提交阶段 7；
2. 冻结 ADR、协议方法、错误模型和敏感字段；
3. 创建 Bridge/Bridge.Tests 和 schema 目录；
4. 打通 hello、initialize、shutdown；
5. 建立合同生成、stdout purity、Dispose 不停 Agent 和架构门禁；
6. 运行完整 .NET 测试并记录阶段 1 完成说明。

此批次不创建可见 React 页面，不修改 IPC、SQLite、Agent 状态机、App.xaml.cs 或现有 WPF 发布流程。阶段 1 稳定后再进入 Agent Bridge 切片；这样可以先把跨语言合同和生命周期的高风险边界做实，再引入 Rust/Tauri/React 工具链。
