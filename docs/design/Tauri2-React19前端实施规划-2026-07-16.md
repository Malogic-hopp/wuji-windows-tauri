...
状态：阶段 0～5 已完成；阶段 6A～6C 自动化实施完成、真实 dev parity 与 soak 待执行；阶段 7～8 待实施
...
当前状态：阶段 6A～6C 已于 2026-07-17 完成自动化实施。宿主生命周期、close/minimize-to-tray、Settings dirty 原生确认、Bridge graceful/forced shutdown、Agent 四态托盘菜单、单实例、固定 dev 身份反向校验、dev/prod 隔离、WPF/Tauri parity、崩溃/循环恢复和 24 小时资源监测工具均已落地；真实 dev parity smoke、短时资源检查和条件允许时的 24 小时 soak 待执行。详见 `docs/devlog/Tauri2-React19阶段6A完成说明-2026-07-17.md`、`docs/devlog/Tauri2-React19阶段6B完成说明-2026-07-17.md` 和 `docs/devlog/Tauri2-React19阶段6C完成说明-2026-07-17.md`。
...
59. 阶段 6C PowerShell 5.1 parser/BOM、合同漂移、TypeScript、ESLint、React 39 项、production build、bundle budget、Rust 25 项/Clippy、Tauri 架构 30 项和 .NET full suite 660 项均通过；真实 parity 与 soak 待执行，详见 `docs/devlog/Tauri2-React19阶段6C完成说明-2026-07-17.md`。

下一步先执行 **`pnpm smoke:lifecycle-parity`**，完成阶段 6A～6C 的真实 dev 合并验收；通过后执行 15 分钟资源检查，条件允许时再执行 `pnpm soak:lifecycle:24h`。只有 parity 与所需 soak 报告通过后才关闭阶段 6 并进入阶段 7。继续不修改现有 WPF 默认 Shell、IPC、SQLite、Agent 状态机或 WPF 发布流程。
