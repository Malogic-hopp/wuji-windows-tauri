...
- **统计主页工作节奏（Work Pace）已实现（2026-08-07，产品确认收进 v0.1 小改进，非 v0.2）。** 惯性卡片新增工作/未工作占比例条 + 常见工作时段 + 上午利用率：`workPace` 字段随 `stats_get_home` 返回（不新增命令）。口径（设计 10 §4.4 增补 + 09 §10.6 已回写）：工作 = Work Block 覆盖并集（含块内短 idle）、未工作 = 24h 补集，占比自洽；仅有效记录日参与均值（同惯性窗口与 reliability 门禁）；**本地 00:00–06:00 开始的块一律归前一天**（熬夜语义——人不会凌晨开工，即使 Agent 断连也按熬夜归前日；真实数据中 02:02–02:36、03:05–03:06 凌晨块正确归入前日，消除"凌晨开工"伪值）；常见开工/收工 = 当天窗口内真实覆盖段上中位数（熬夜尾巴不参与开工、收工截到 24:00）；`morningWorkDays` = 8-12 点有覆盖的有效日数；午休低谷增加前置条件"11 点有实质活跃才标注"（上午未开工时 12-14 点低谷是"未开工"而非午休，修复真实数据误标"午休低谷 12 点"）。真实数据验证：工作 11%（日均 2h38m）· 未工作 89%、常见工作时段 16:18–23:02、上午 0/9 天、凌晨 0-3 点覆盖如实反映为熬夜。实现：`wuji-storage::Reader::stats_work_pace_days`（凌晨/跨午夜归属、窗口裁剪）+ `wuji_core::stats::derive_work_pace`（中位数/上午判定/熬夜映射，单测 6 项）+ `derive_inertia` 午休低谷前置条件（单测 1 项）；storage 集成测试跨午夜归属 1 项；bindings 双副本无 drift；前端 InertiaCurve 占比条（未工作段 `--chart-no-data` 填充）+ 工作时段/上午文字，移除叠加色带（用户明确不要），行距统一 8px。门禁：wuji-core 63、storage 45 全绿（desktop 测试因 `wuji-rebuild-desktop-v01.exe` 被桌面进程占用待补跑）、`pnpm typecheck`/`lint`/`test` 146 项/`build` 全绿。探针工具 `wuji-storage/examples/dump_work_pace.rs`（只读诊断）随实现保留。
