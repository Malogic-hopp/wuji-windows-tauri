...
| React 19 UI 基座 | Implemented | `apps/desktop`：Today/Timeline/Settings/Diagnostics 四页 + 四态 + 顶栏 Agent 控制 + 令牌主题 + forced-colors 适配；30 项 Vitest 通过；Timeline 日期使用 DB reporting 时区（R08），5s 轮询整体重取（UI 不分页，最新在顶部，含到底/到顶悬浮按钮；超长一天明确截断提示，轮询防重入）；Diagnostics 时间基准随轮询更新（R09） | Today/Timeline/Settings/Diagnostics 使用 v0.1 DTO | 手工矩阵（尺寸/DPI/HC/键盘/读屏）Pending | 手工矩阵 |
