...
状态：阶段 0～5 已完成；阶段 6A～6C 自动化、真实 dev parity、15 分钟和 60 分钟 soak 已通过，条件允许时的 24 小时 soak 待执行；阶段 7～8 待实施
...
实施状态（2026-07-17）：已完成。4A 合同与 Bridge、4B Rust command、4C React Dashboard 和 4D parity/验收全部通过。真实 dev 数据已通过固定 Bridge 与 WPF 查询链路对照；确定性非空 SQLite 用例补齐 Top Apps 排序/时长、最近会话和跨本地午夜语义；Empty/Error、隐私 allowlist、route chunk 与主 bundle 预算均已建立门禁。详见 `docs/devlog/Tauri2-React19阶段4A完成说明-2026-07-16.md`、`docs/devlog/Tauri2-React19阶段4B完成说明-2026-07-16.md`、`docs/devlog/Tauri2-React19阶段4C完成说明-2026-07-16.md`、`docs/devlog/Tauri2-React19阶段4D完成说明-2026-07-17.md`。
...
当前状态：阶段 5A～5D 已于 2026-07-17 完成。Settings 合同、Bridge、Rust commands、React 表单、WPF/Tauri parity、断线恢复、隐私和 bundle 验收均已通过；详见 `docs/devlog/Tauri2-React19阶段5D完成说明-2026-07-17.md`。
...
当前状态：阶段 6A～6C 已于 2026-07-17 完成自动化实施，真实 dev parity、15 分钟检查和 60 分钟稳定性复核均已通过。宿主生命周期、close/minimize-to-tray、Settings dirty 原生确认、Bridge graceful/forced shutdown、Agent 四态托盘菜单、单实例、固定 dev 身份反向校验、dev/prod 隔离、WPF/Tauri parity、崩溃/循环恢复和 24 小时资源监测工具均已落地；60 分钟分段样本确认 Tauri/Bridge/Agent memory 与 handle 已进入平台期，条件允许时可执行 24 小时 soak。详见 `docs/devlog/Tauri2-React19阶段6A完成说明-2026-07-17.md`、`docs/devlog/Tauri2-React19阶段6B完成说明-2026-07-17.md` 和 `docs/devlog/Tauri2-React19阶段6C完成说明-2026-07-17.md`。
...
18. React Dashboard 已实现 Loading/Empty/Ready/Error、摘要、Top Apps、最近会话、最后更新时间、手动刷新与安全错误重试；
19. 秒数与 UTC 时间仅在表现层做 locale-aware 显示，Top Apps 顺序和业务指标不在 React 重算；
20. 页面可见时每 15 秒刷新，隐藏时降至 60 秒；Bridge ready 继续使 Overview query 失效；
21. 阶段 4C React typecheck/lint/18 项测试、production build、Rust 8 项测试/Clippy、14 项 Tauri 架构测试、626 项 full suite 与合同漂移检查通过，详见 `docs/devlog/Tauri2-React19阶段4C完成说明-2026-07-16.md`。
22. 固定 dev parity 工具已用同一数据库对照 WPF `DailyStats/Sessions` 与真实 Bridge `activity.getOverview`，并拒绝 Agent 运行期间的非原子读取；
23. 真实 dev 数据的今日摘要、最近会话、日期边界、本地时区和隐私 allowlist 全部通过；确定性非空 Integration 用例补齐 Top Apps 排序/时长与跨午夜缩放；
24. 主 bundle 为 343.46/107.23 KiB raw/gzip，Dashboard route 为 19.36/5.44 KiB，Settings route 为 4.48/1.79 KiB，全部低于固定预算；
25. 阶段 4D React 18 项、Rust 8 项、Dashboard/Tauri 架构 17 项与 .NET full suite 629 项通过，详见 `docs/devlog/Tauri2-React19阶段4D完成说明-2026-07-17.md`。
26. Settings v1 已冻结为主题、刷新间隔、UI 自动启动 Agent，以及 Agent 五个数值参数和四个布尔开关；登录启动、托盘、排除列表、路径、注册表、数据库和任意键值接口均不进入合同；
27. `settings.get/settings.update`、安全 Settings DTO、三端生成类型、Application 读取/校验/保存用例和 Bridge 显式 mapper 已完成；Bridge 只通过 `IWujiClient.Settings` 访问设置；
28. 更新只覆盖 allowlist 字段，未开放字段保持原样；无效值不写入，App 保存失败会尝试恢复 Agent backup，字段错误只返回固定字段名和固定安全文案；
29. 阶段 5A 合同漂移、TypeScript、Rust 8 项测试/Clippy、Application 9 项、Bridge 46 项、架构门禁与 646 项 .NET full suite 均通过，详见 `docs/devlog/Tauri2-React19阶段5A完成说明-2026-07-17.md`。
30. Rust 已增加固定语义 `settings_get/settings_update`，只使用生成的 `SettingsGetResult/SettingsUpdateParams/SettingsUpdateResult` 与 BridgeSupervisor typed request；
31. Supervisor 支持带参数请求并原样转发 Settings DTO；读取使用 12 秒外层 timeout，保存使用 18 秒外层 timeout，保存超时不可盲目重试；
32. Rust invoke handler 与 React command 白名单均扩展为十个固定命令，Bridge validation field errors 可安全透传，Bridge ready 后会同时失效 Dashboard 与 Settings query；
33. 阶段 5B React typecheck/lint/19 项测试、Rust 11 项测试/Clippy、19 项 Tauri 架构测试、合同漂移与 647 项 .NET full suite 均通过，详见 `docs/devlog/Tauri2-React19阶段5B完成说明-2026-07-17.md`。
34. `settings.get` 现在同时返回当前安全快照与 Core/Application 默认快照；React 的“恢复默认值”只填充生成 DTO 草稿，不复制默认常量且不自动落盘；
35. React Settings 已实现 Loading、Ready、Saving、Success、Error，四个分组覆盖全部十二个 allowlist 字段，并支持本地整数格式校验、Bridge 字段错误、安全重试和成功后 query 刷新；
36. dirty 状态、`beforeunload`、React Router blocker、可聚焦 `alertdialog`、Esc/Tab 焦点循环、放弃修改、恢复默认值和重复提交门禁已完成；
37. Settings 使用现有语义 token，支持 Light/Dark、forced-colors High Contrast、reduced-motion、原生 label/fieldset/select/checkbox、live region 和 locale-aware 整数展示；React 没有文件、注册表、SQLite 或通用 invoke 能力；
38. 阶段 5C 合同漂移、TypeScript、ESLint、React 30 项、production build、bundle budget、Rust 11 项/Clippy、20 项 Tauri 架构和 650 项 .NET full suite 均通过，详见 `docs/devlog/Tauri2-React19阶段5C完成说明-2026-07-17.md`。
39. 阶段 5D 新增可重复的 Settings parity 工具和 `pnpm smoke:settings-parity`；使用唯一临时 data root、固定 dev channel、真实 WPF `SettingsViewModel` 与真实 Bridge 双向对照，并在 finally 恢复和清理工作区；
40. 初始值、Core/Application 默认值、Tauri→WPF 与 WPF→Tauri 合法 round-trip、Bridge/UI 重启持久化、双方非法拒绝和 Bridge 崩溃后重连均通过；
41. Settings 保存成功会同时失效 Settings、Agent status 和 Dashboard query；不新增隐式 Agent reload，继续保持 WPF 普通保存只落盘的既有语义；
42. 真实 prod 设置前后指纹不变；成功和错误 DTO 均不包含路径、数据库、注册表、IPC、自由文本隐私字段或异常详情；
43. 阶段 5D React 33 项、WPF Settings 50 项、parity 19 项、Tauri 架构 21 项、Rust 11 项和 .NET full suite 651 项均通过；主 bundle 344.47/107.52 KiB、Dashboard 19.36/5.44 KiB、Settings 16.58/5.08 KiB raw/gzip，详见 `docs/devlog/Tauri2-React19阶段5D完成说明-2026-07-17.md`。
44. 阶段 6A 新增 Rust `HostLifecycle`，冻结 Visible、HiddenToTray、ExitConfirmationPending、ShuttingDown、Exited 状态及 close-to-tray/true-exit 语义；托盘骨架仅开放固定“显示吾迹”和“退出吾迹”；
45. Settings dirty 通过五个固定 lifecycle command 与 `host://close-requested` 安全事件接入原生窗口关闭；React alertdialog 支持安全默认焦点、Esc、Tab/Shift+Tab，并明确 UI 退出不停止 Agent；
46. Bridge shutdown 现在区分 graceful/forced/already-exited，等待 supervisor worker 结束，超时执行 kill + wait/reap，并保留 kill-on-drop 兜底；宿主退出路径不调用 `agent.stop`；
47. 阶段 6A 合同漂移、TypeScript、ESLint、React 38 项、production build、bundle budget、Rust 18 项/Clippy、Tauri 架构 24 项和 .NET full suite 654 项均通过；真实 dev 托盘 smoke 待人工执行，详见 `docs/devlog/Tauri2-React19阶段6A完成说明-2026-07-17.md`。
48. 阶段 6B 托盘扩展为 Agent 状态、start/pause/resume/stop、显示、隐藏与退出的固定原生菜单；操作期间禁用冲突项，完成后立即刷新，并以 4 秒间隔和 missed-tick skip 避免刷新风暴；
49. 冷启动 running、paused、stale、not-running 均有独立中文状态和最小可用动作；托盘不暴露 PID、路径、数据库、窗口标题、内部异常或任意命令参数；
50. Tauri dev Shell 使用固定 `Local\WUJI.Tauri.Dev.SingleInstance.v1` mutex；第二实例不创建 Bridge/WebView，只通过 Win32 恢复并激活标题固定的已有窗口后退出；
51. BridgeSupervisor 固定传入 `--channel dev`，并在 initialize 后反向校验 `channelName == dev` 与非默认 channel；React 无 channel、可执行文件路径或数据根输入；
52. 架构门禁证明 dev/prod pipe、Agent mutex、数据根和启动项 value name 不同，Tauri 单实例 mutex 不复用 Agent mutex，且未引入 autostart plugin；
53. 阶段 6B 合同漂移、TypeScript、ESLint、React 39 项、production build、bundle budget、Rust 23 项/Clippy、Tauri 架构 27 项和 .NET full suite 657 项均通过；测试未启动真实 Agent、未访问或修改 prod，详见 `docs/devlog/Tauri2-React19阶段6B完成说明-2026-07-17.md`。
54. 阶段 6C 对照 WPF 默认 `MinimizeToTray=true` 补齐 Tauri minimize-to-tray；最小化保留 dirty 草稿且不弹关闭确认，已打开的关闭确认不能被最小化绕过；
55. 新增 `pnpm smoke:lifecycle-parity`，按固定 dev 顺序验证 WPF/Tauri close/minimize-to-tray、UI 退出后 Agent 存活、显式 stop、Bridge/Tauri 崩溃恢复、无孤儿 Bridge、无重复 Agent及三次启动/退出循环；
56. 崩溃注入只允许结束经过进程名、parent PID、唯一 dev Bridge 和 `--channel dev` 复核的 Tauri/Bridge；脚本不按名称结束进程、不结束 Agent、不接受任意 channel/path/PID；
57. 新增 `pnpm soak:lifecycle` 与 `pnpm soak:lifecycle:24h`，只读采样 Tauri/Bridge/Agent 的 working set、private memory、handle、CPU 和 I/O，并检查固定 PID、孤儿/重复进程、内存/句柄增长与倍增型刷新风暴；
58. 24 小时入口固定 1440 分钟、60 秒采样、5 分钟 warmup 和运行中 Agent；托盘 `4s + missed-tick skip`、React Agent `4s`、Dashboard `15s/60s` 由静态门禁固定；
59. 阶段 6C PowerShell 5.1 parser/BOM/Add-Type、合同漂移、TypeScript、ESLint、React 39 项、production build、bundle budget、Rust 25 项/Clippy、Tauri 架构 30 项和 .NET full suite 660 项均通过；
60. 真实 `lifecycle-parity-smoke` 已通过：WPF/Tauri close/minimize-to-tray、UI/Agent 生命周期独立、Bridge/Tauri 崩溃恢复、三次启动/退出循环、显式 stop、孤儿/重复进程检查及 prod Agent/pipe 不变量全部 PASS；长期资源 soak 待执行，详见 `docs/devlog/Tauri2-React19阶段6C完成说明-2026-07-17.md`。
61. 真实 15 分钟资源检查已通过：Tauri/Bridge/Agent 拓扑稳定，Tauri memory/handle 基本稳定，Bridge 增长 9.35 MiB/42 handles，Agent 增长 7.45 MiB/53 handles，I/O 中位数/峰值为 345.16/430.32 ops/min 且无倍增型刷新风暴，prod Agent/pipe 基线不变；因 Bridge/Agent memory 尚未明确进入平台期，先追加 60 分钟复核。
62. 真实 60 分钟稳定性复核已通过：Tauri 增长 0.17 MiB/5 handles，Bridge 增长 0.06 MiB/-12 handles，Agent 增长 -0.02 MiB/29 handles，I/O 中位数/峰值为 346.3/412.03 ops/min；四分位样本确认 Bridge memory 稳定在约 30.43～30.46 MiB，Agent memory 在 28.72～29.61 MiB 内波动，无持续增长、孤儿 Bridge、重复 Agent 或 prod 基线变化。

下一步在条件允许时执行 **`pnpm soak:lifecycle:24h`**。若机器无法维持 24 小时登录与不休眠环境，则明确记录环境原因延期；长期 soak 完成或正式延期前不把该项标记为通过。继续不修改现有 WPF 默认 Shell、IPC、SQLite、Agent 状态机或 WPF 发布流程。
