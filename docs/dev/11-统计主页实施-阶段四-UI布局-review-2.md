有，**提升很明显**。上一版更像“统计报告页”，这一版已经更接近真正的产品主页：首屏能先回答“今天怎么样、本周怎么样”，随后再逐层展开近期、周期、行为模式和长期记录。

从截图和当前代码看，之前提出的主要结构问题基本已经解决，**不建议再推翻整体布局**。接下来应以交互语义和局部视觉收口为主。

## 这版提升最明显的地方

### 1. 首屏主次关系已经成立

“今日状态 + 本周进度”合并到一张主卡后明显更好：
...
`2.4fr / 0.8fr` 的比例也比原来的 1∶1 合理得多。

### 2. 两张趋势图真正成为了页面骨架

现在的顺序很清楚：

> 今日 → 近 14 天 → 近 12 周 → 行为结构 → 长期记录

近 14 天和近 12 周都使用开放式区块，没有继续套卡片，页面轻了很多。补充的日期锚点、月份刻度、日均与周均摘要也解决了此前“只能看形状、不能判断量级”的问题。
...
这类细节使页面看起来可信，而不是单纯“画得漂亮”。

### 4. 工作惯性和应用构成比之前清楚

“工作惯性”的副标题已经解释了指标含义，因此现在不必急着改名。应用构成增加每日总时长后，也不会再让人误以为所有等宽横条代表相同的活跃量。

---

# 仍建议优先处理的四个问题

## 1. 7 / 14 / 30 天切换器的控制范围不够直观

当前切换器放在“活跃趋势”标题右侧，但代码中它同时改变：

* 活跃趋势；
* 下方应用构成。

用户点击这里时，很容易认为只会改变紧邻的趋势图。尤其切换到 30 天后，应用构成还会从“每日横条”变成“每周纵柱”，属于比较大的图形变化，但控制器距离它很远。
...
* 当前切换器只控制活跃趋势；
* 应用构成固定为近 14 天；
* 后续确实需要切换时，在应用构成卡片内部放自己的范围控制。

**方案 B：保持联动，但把切换器移到页面标题区，并明确标注“近期范围”。**

不建议继续保持现在这种“局部位置、跨区块生效”的状态。

---

## 2. 主卡还有一点“文字排版感”，右侧数字不够突出

目前左侧的“8 个工作块”因为 `margin-left: auto` 被推到了分隔线旁，看起来有一点漂浮，容易被误认为属于本周区域。
...
其中 `16h12m` 可以使用约 `20–22px`，小于今日的 `30px`，但明显大于说明文字。这样主卡会形成更稳定的双指标结构。

此外，截图中的：

> 较上周同期 新增 16 小时 12 分钟

与主数字的 `16h12m` 格式不一致，也比较占宽。主页中统一成紧凑格式更合适：
...
日期是这一行的类别标签，放在条形之后不太符合从左到右的扫描习惯。建议改成：

```text
07-05  [构成条]  3h30m
```

这样用户可以先沿左侧日期纵向扫描，再观察构成，最后读取总量。

可以直接用 Grid 调整，不必重写组件结构：

```css
.comp-row {
  display: grid;
  grid-template-columns: 44px minmax(0, 1fr) 48px;
  grid-template-areas: 'date bar total';
  gap: var(--space-2);
  align-items: center;
}
...
无记录日右侧可以显示 `—`，有记录但零活跃则继续显示 `0m`，进一步强化两种状态的区别。

---

## 4. 左右两张行为卡的高度差仍然比较明显

实际截图中，应用构成卡比工作惯性卡高出一截。现在使用 `align-items: start`，因此左卡提前结束，右卡继续向下，双列区域的底边不齐。

这不是严重问题，但会削弱仪表盘的整齐感。建议：
...
同时将惯性图从 `140px` 提高到约 `170px`，并把底部信息整理成两行或两列，而不是单纯留下空白。
...
现在只有 `6 / 12 / 18 / 24`，图表左端缺少时间起点，第一段空白看起来稍微没有依据。
...
当前方案不难看，而且有短期—周期—长期的层次感；但紫色同时代表“周趋势”和“浏览器”，绿色同时代表“月趋势”和“终端”，颜色语义略有交叉。

更严谨的方案是：

* 所有“活跃时长”图表使用同一蓝色体系，通过明度区分；
* 蓝、紫、绿只用于应用类别；
* 橙色斜纹继续专门表示“进行中”。

这项不是必须修改，但会让整套视觉语言更稳定。
...
```tsx
style={{ color: slotToToken(entry.slot) }}
```

同时给色块和文字上色。更专业的处理通常是：

* 色块使用类别颜色；
* 文字统一使用 `var(--text-dim)`。

这样图例不会形成一排彩色文字，页面会更克制。
...
> 累计记录 138 天 · 始于 2026 年 3 月 · 最长连续记录 67 天
> 近 6 个月日均活跃（按有效记录日）

“最长连续数据区间”稍显技术化，“最长连续记录”更符合产品语言。

### 顶栏的“停止 Agent”可以弱化

“暂停”是高频操作，“停止 Agent”更像维护或诊断操作，两者目前视觉等级接近。正式产品中更建议把“停止 Agent”放进更多菜单或诊断页，顶栏保留暂停/继续即可。

---

## 最终判断

这版已经可以作为**主页静态布局的主体定稿**，不需要继续增加卡片、图标、阴影、网格线或额外指标。最值得在进入真实数据接入前处理的是：
...
完成这几项后，页面会从“结构已经正确”进一步达到“交互预期清晰、视觉收口完整”的状态。截图下方的大面积留白不需要刻意填充，桌面窗口高于内容高度时保留空白比强行加入低价值模块更
